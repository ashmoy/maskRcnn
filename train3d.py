    ...
    for batch_idx, (images, targets) in enumerate(test_loader):
        for img_idx, image in enumerate(images):
            ...
            if image.dim() == 3:  # Si c'est un volume 2D [C, H, W]
                ...
                # S'assurer que l'image est sur le bon appareil et nécessite des gradients
                image = image.to(device).requires_grad_(True)

                with torch.set_grad_enabled(True):  # Activer les gradients temporairement
                    integrated_gradients = IntegratedGradients(model)
                    output = model(image.unsqueeze(0))  # Ajouter la dimension batch
                    print(f"Output: {output}")

                    if isinstance(output, list) and len(output) > 0:
                        output = output[0]

                    # Vérifier si la sortie du modèle nécessite un gradient
                    if output['scores'].requires_grad:
                        print("Scores tensor requires grad")


                    else:
                        print("Scores tensor does not require grad")

                    # Extraire les scores et labels

                    if output['labels'].requires_grad:
                        print("Labels tensor requires grad")
                    ...
                    if image.dim() == 3:  # Doit être au format (C, H, W)
                        # Appliquer Integrated Gradients uniquement sur les entrées nécessitant un gradient
                        attributions = integrated_gradients.attribute(
                            inputs=image.unsqueeze(0),  # Ajouter la dimension batch
                            target=target_label,
                            n_steps=steps
                        )

                        # Sauvegarde ou visualisation des attributions ici
                        # ...
                    else:
...
def extract_valid_slices(scan_file, mask_file):
    try:
        ...
        print(f"Error loading files {scan_file} or {mask_file}: {e}")
        return [], []
    ...
    for i in range(min_slices):
        ...
        # Libérer les slices de la mémoire après leur traitement
        del mask_slice, scan_slice
...
class CanineNiftiValidSliceDataset(Dataset):
    def __init__(self, scan_folder, mask_folder, transforms=None):
        ...
        self.scan_files = sorted([f for f in os.listdir(scan_folder) if f.endswith('.nii')])
...
def train_model(scans_folder, masks_folder, num_epochs, lr, device, output_folder):
    ...
    dataset = CanineNiftiValidSliceDataset(scans_folder, masks_folder, transforms=None)
    ...
    train_loader = DataLoader(train_dataset, batch_size=2, shuffle=True, collate_fn=utils.collate_fn, num_workers=8)
    val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False, collate_fn=utils.collate_fn, num_workers=8)
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, collate_fn=utils.collate_fn, num_workers=8)
